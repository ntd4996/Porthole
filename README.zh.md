...
原生 Linux 版本（Rust + GTK4/libadwaita）位于 [`linux/`](linux/)。

```bash
# 一行命令：安装最新 AppImage 到 ~/.local/bin 并注册到应用菜单
curl -fsSL https://raw.githubusercontent.com/ntd4996/Porthole/main/linux/install.sh | bash
```

或从 [发布页面](https://github.com/ntd4996/Porthole/releases) 手动获取构建（标签 `linux-v*`）：
...
或用一行命令从 **APT 软件源** 安装（之后 `apt upgrade` 自动更新）：

```bash
curl -fsSL https://raw.githubusercontent.com/ntd4996/Porthole/main/linux/setup-apt.sh | sudo bash
```

该命令会添加已签名的软件源并安装 `porthole`。（想手动添加软件源？见 [`linux/README.md`](linux/README.md)。）
